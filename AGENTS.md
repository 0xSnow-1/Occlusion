...
- Ingest pipeline: `uv run python scripts/run_ingest.py` (default corpus from `data/raw` plus the 11 `DEFAULT_HTML_URLS`, into on-disk Qdrant at `./data/qdrant_storage`; `--pdf-only` for local PDFs only).
